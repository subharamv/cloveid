...
const BulkCardEditor: React.FC = () => {
    ...
    const { rowData, headers, rowIndex, csvData } = location.state || { rowData: [], headers: [], rowIndex: -1, csvData: [] };
    ...
    useEffect(() => {
        ...
        if (rowData && headers) {
            ...
            headers.forEach((header: string, index: number) => {
                ...
                const employeeKey = headerMapping[key as keyof typeof headerMapping];
                if (employeeKey) {
                    newEmployee[employeeKey] = rowData[index];
                }
                if (key.toLowerCase() === 'photo' || key.toLowerCase() === 'image' || key.toLowerCase() === 'photo (upload)') {
    ...
    const handleSaveAndBack = async () => {
        ...
        try {
            toast.info('Generating ZIP file before going back...');
            ...
            navigate('/import-management', { state: { updatedEmployee: employee, rowIndex, zipUrl, csvData, headers } });
        } catch (error) {
            console.error('Error generating ZIP on back navigation:', error);
            toast.error(`Failed to generate ZIP file: ${error instanceof Error ? error.message : 'Please try again.'}`);
        }
    };
