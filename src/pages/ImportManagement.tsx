...
const ImportManagement = () => {
                ...
                if (cards && cards.length > 0) {
                    ...
                    cards.forEach((card, idx) => {
                        ...
                        if (card.print_status) {
                            ...
                            loadedPrintStatuses[idx] = card.status;
                        }
                    });
    ...
    const handleSaveBatch = async () => {
        ...
        try {
            ...
            for (let i = 0; i < csvData.length; i++) {
                ...
                if (existingCardId) {
                    cardPayload.id = existingCardId;
                } else {
                    cardPayload.status = 'pending';
                    cardPayload.created_by = user.id;
                }
    ...
    const handleDeleteSelectedRows = () => {
        ...
        if (!window.confirm(`Are you sure you want to delete ${selectedRows.size} selected cards?`)) {
            return;
        }
        ...
        csvData.forEach((row, oldIdx) => {
            if (selectedRows.has(oldIdx)) {
                if (cardIds[oldIdx]) {
                    newDeletedIds.push(cardIds[oldIdx]);
                }
            } else {
                newCsvData.push(row);
    ...
    const handleDeleteRow = (index: number) => {
        ...
        const newDeletedIds: number[] = [];
        if (cardIds[index]) {
            newDeletedIds.push(cardIds[index]);
        }
