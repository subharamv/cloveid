...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        const timeoutId = setTimeout(() => {
            console.warn('Dashboard: Data fetching timed out after 15s, aborting...');
            controller.abort();
        }, 15000);
        ...
        try {
            ...
            const queries = [
                supabase.from('requests').select('*').order('created_at', { ascending: false }).limit(5).abortSignal(controller.signal),
                supabase.from('card_batches').select('*').order('created_at', { ascending: false }).limit(5).abortSignal(controller.signal),
